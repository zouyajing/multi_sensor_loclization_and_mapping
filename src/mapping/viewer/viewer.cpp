...
bool Viewer::JointCloudMap(const std::deque<KeyFrame>& key_frames, CloudData::CLOUD_PTR& map_cloud_ptr) {
    ...
    for (size_t i = 0; i < key_frames.size(); ++i) {
        // std::cout << "insert key frame " << i << std::endl;
...
bool Viewer::SaveMap() {
    std::cout << "Start to save map" << std::endl;
    std::cout << "key frame size: " << optimized_key_frames_.size() << std::endl;
    if (optimized_key_frames_.size() == 0)
        return false;
    // 生成地图
    CloudData::CLOUD_PTR global_map_ptr(new CloudData::CLOUD());
    JointCloudMap(optimized_key_frames_, global_map_ptr);
    std::cout << "map size: " << global_map_ptr->points.size() << std::endl;
    // 保存原地图 <
