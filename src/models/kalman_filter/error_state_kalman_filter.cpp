...
ErrorStateKalmanFilter::ErrorStateKalmanFilter(const YAML::Node &node) {
  ...
  // c. 过程噪声: n_a n_omega n_ba n_bomega
...
void ErrorStateKalmanFilter::GetOdometry(Eigen::Matrix4f &pose,
  ...
  pose_double.block<3, 3>(0, 0) = pose_double.block<3, 3>(0, 0) * (Eigen::Matrix3d::Identity()- C_nn);

  Eigen::Quaterniond q(pose_double.block<3, 3>(0, 0));
  q.normalize();
  pose_double.block<3, 3>(0, 0) = q.toRotationMatrix();
...
bool ErrorStateKalmanFilter::GetVelocityDelta(
  ...
  velocity_delta = T * (GetUnbiasedLinearAcc(linear_acc_curr, R_curr) + GetUnbiasedLinearAcc(linear_acc_prev, R_prev))/2;
...
void ErrorStateKalmanFilter::UpdateProcessEquation(
  ...
  Eigen::Vector3d f_n = linear_acc_mid;
...
void ErrorStateKalmanFilter::EliminateError(void) {
  ...
  // c. 姿态（这个公式与ppt稍有不同,由于对于姿态的误差的计算方式）:
  Eigen::Matrix3d C_nn = Sophus::SO3d::hat(X_.block<3, 1>(kIndexErrorOri, 0)).matrix();
